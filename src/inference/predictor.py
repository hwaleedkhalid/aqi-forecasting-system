...
class AQIPredictor:
    ...
    def predict_latest(
        ...
        if path.exists():
            # Read the latest rows from CSV dataset
        ...
        elif self.runtime_resolver.get_bootstrap_feature_path().exists():
            # Fallback to committed canonical bootstrap feature vector
            logger.info("Loading latest telemetry from runtime bootstrap feature vector...")
            df_features, metadata = self.runtime_resolver.load_bootstrap_feature_vector()
            input_observed_at = metadata["input_observed_at"]
            current_aqi = metadata["current_aqi"]
        else:
            raise FileNotFoundError(f"Feature dataset not found at {path} and bootstrap vector missing.")
    ...
    def get_latest_observation(
        ...
        if path.exists():
            ...
            _, metadata = self.runtime_resolver.load_bootstrap_feature_vector()
            observed_time = metadata["input_observed_at"]
            current_aqi = metadata["current_aqi"]
            dominant = metadata["dominant_pollutant"]
    ...
    def explain_latest(
        ...
        if path.exists():
            ...
            df_boot, _ = self.runtime_resolver.load_bootstrap_feature_vector()
            raw_vec = df_boot.values
            schema = self.model_loader.load_schema()
        else:
            raise FileNotFoundError(f"Feature dataset not found at {path} and bootstrap vector missing.")
