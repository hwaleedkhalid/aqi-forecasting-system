...
*   **Scheduled Hourly Live Feature Pipeline**: The hourly pipeline is implemented and manually validated; scheduled GitHub Actions production execution is pending final secret configuration.
    ...
    *   *Authoritative Current AQI*: Computed via standard US EPA piecewise linear interpolation (`calculate_overall_aqi`), cleanly distinguishing current observation $T$ (`epa_aqi=120`) from $T-1\text{h}$ lag (`epa_aqi_lag_1h=114`).
    *   *Dual-Store Ingestion Semantics*: Historical event is ingested to offline Hudi storage with `start_offline_materialization=True` (idempotent upsert), while the online RonDB store is updated with `upsert_if_newer=True`.
    *   *Automated Orchestration & Security*: Scheduled via GitHub Actions (`.github/workflows/feature_pipeline.yml`) on cron `17 * * * *` with concurrency control (`cancel-in-progress: false`), least privilege permissions (`contents: read`), fail-fast secret validation, and structured snapshot export to `data/snapshots/latest_ingestion_report.json`. Dependency isolation is guaranteed via `requirements-feature-pipeline.txt` (`hopsworks==5.0.6`), preserving the clean deployment footprint of Render and Streamlit.
